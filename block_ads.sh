...
curl -sSfL --retry "$MAX_RETRIES" --retry-all-errors \
  ...
  | grep -vE '^\s*(#|$)' > oisd_big.domains.txt || silent_error "Failed to download the domains list"
...
# === Split into chunks ===
split -l ${MAX_LIST_SIZE} oisd_big.domains.txt oisd_big.domains.txt. || error "Failed to split list"
...
git add oisd_big.domains.txt || error "Failed to add list"
